...
#### مشكلة Git Push - "src refspec main does not match any"
```bash
# السبب: الـ remote يسمى شيء آخر غير origin أو الفرع مختلف
# الحل:
git remote -v  # تحقق من اسم الـ remote
git branch -a  # تحقق من الفروع المتاحة
git push [remote-name] [current-branch]  # استخدم الأسماء الصحيحة
```

#### مشكلة الملفات الكبيرة في Git
```bash
# إذا فشل push بسبب ملفات كبيرة (>100MB)
...
#### إنشاء فرع نظيف للنشر
```bash
# إذا كان التاريخ ملوث بملفات كبيرة
git checkout -b clean-deployment [last-good-commit]
git checkout [old-branch] -- [files-you-need]
git add .
git commit -m "Clean deployment branch"
git push [remote-name] clean-deployment
```
